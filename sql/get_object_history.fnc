...
BEGIN
 ...
 LOOP
	 ...
	 thesaurus_record rec
	  ...
	  JOIN thesaurus_name rec_name ON r_ref.record_from_qualifier = rec_name.qualifier
	 ...
	 WHERE r_ref.record_from_qualifier = forward_neighbor
	  AND begin_doc.document_date <= date_end
	  AND end_doc.document_date >= date_begin;
	 ...
	 EXIT WHEN (back_neighbor ISNULL AND forward_neighbor ISNULL);

 END LOOP;
 
 RETURN QUERY SELECT * FROM result_list;
